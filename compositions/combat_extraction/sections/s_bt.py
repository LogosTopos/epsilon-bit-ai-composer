...
- hook → 时间晶体:每 2 小节 1-2 音高区闪烁(vel 64-72,母节 HOOK_HIGH /
  HOOK_PHRASE 原位,69-81 带);cycle1 单音小节补第 2 音(两圈微变防机械)
...
# 时间晶体音高(全部母节 hook 素材原位,69-81 带):
#   CRY_HIGH = 母节 HOOK_HIGH 表(轮 2/4 句头高音);其中 C 的 77=F5 母节从未发声且非 C 五声,
#   已改用母节验证过的 79=G5(和弦音);CRY_LOW = HOOK_PHRASE bar1(句头长音);
#   CRY_ALT  = HOOK_PHRASE bar3[1](句尾长音,轮 2/4 轮头作第二音,防机械)
CRY_HIGH = {'Em': 81, 'C': 79, 'G': 79, 'D': 78}
CRY_LOW = {'Em': 76, 'C': 72, 'G': 71, 'D': 74}
CRY_ALT = {'Em': 78, 'C': 76, 'G': 76, 'D': 74}
...
def build(s, bar0, cycle, ch):
    ...
    # ---------------- hook 时间晶体:每 2 小节 1-2 音高区闪烁(母节素材原位) ----------------
    for rel in CRY_RELS:
        prog = CHORDS16[rel]
        hi = CRY_HIGH[prog]
        t = bt(bar0 + rel)
        if rel % 4 == 0:                         # 轮头(rel 0/4/8/12):2 音(高音 + 句头/句尾音)
            second = CRY_LOW[prog] if (rel // 4) % 2 == 0 else CRY_ALT[prog]
            s.note('hook', hi, 72, t + 2.5, 0.45)
            s.note('hook', second, 66, t + 3.5, 0.5)
        elif cycle == 0:                         # 轮中(rel 2/6/10/14):1 音
            s.note('hook', hi, 70, t + 2.5, 0.4)
        else:                                    # cycle1:补第 2 音(两圈微变防机械)
            s.note('hook', hi, 70, t + 2.5, 0.4)
            s.note('hook', CRY_LOW[prog], 64, t + 3.5, 0.5)
