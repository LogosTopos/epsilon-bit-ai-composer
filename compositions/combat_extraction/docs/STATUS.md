...
8. **子 Agent 最大作曲缺陷 = ①滥用 stab ②瞎调音高(2026-08 用户判定,双因)**:
   ...
   **红线升级(子节创作)**:**① brass 刺刀应比母节更轻(≤母节 v9 的 62-76)或干脆删除;
   M3 齐奏只保留三件套(timpani/kick/bass);riser 不作加密。
   ② 音高一律用母节已验证素材原位,不发明新音高关系**(不移调/不叠置/不扩展音区),
   子节只做"层开关 + 密度/力度变形 + 乐句重写(人格化)"。
   S4/S5 保留待改(按此红线重做或并入 SDC 后续版本)。
